        ...
        with ui.column().classes("w-full h-full"):
            ...
            with ui.row().classes("w-full gap-2 mb-4"):
                self.path_input = ui.input(
                    "Path", value=self.current_path, on_change=self.navigate_to_path
                ).classes("flex-grow")
    ...
    def render_file_item(self, filename: str):
        ...
        with ui.card().classes("w-full cursor-pointer hover:bg-gray-100"):
            with ui.row().classes("items-center gap-2"):
                ...
                if not is_dir:
                    ui.button(
                        icon="visibility", on_click=lambda f=filename: self.view_file(f)
                    ).props("flat dense")
                    ui.button(
                        icon="edit", on_click=lambda f=filename: self.edit_file(f)
                    ).props("flat dense")
            ...
            async def create():
                if name_input.value:
                    ...
                    result = await file_tools.write_file(
                        file_path=file_path, content=content_input.value
                    )
                    ui.notify(
                        result,
                        type="positive" if "Successfully" in result else "negative",
                    )
                    await self.load_files()
                    dialog.close()
    ...
    async def edit_file(self, filename: str):
        ...
        try:
            ...
            with ui.dialog() as dialog, ui.card().classes("w-96"):
                ...
                async def save():
                    result = await file_tools.write_file(
                        file_path=file_path, content=editor.value
                    )
                    ui.notify(
                        result,
                        type="positive" if "Successfully" in result else "negative",
