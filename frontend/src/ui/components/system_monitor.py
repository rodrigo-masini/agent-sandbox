...
class SystemMonitorComponent:
    ...
    async def render_overview(self):
        ...
        with ui.column().classes("w-full gap-4"):
            ...
            ui.label("Resource usage charts would be displayed here").classes(
                "text-gray-500"
            )
    ...
    async def render_processes(self):
        ...
        with ui.column().classes("w-full"):
            ...
            with ui.row().classes("gap-2 mb-4"):
                ui.select(
                    ["cpu", "memory", "pid", "name"], value="cpu", label="Sort by"
                )
        ...
        with ui.column().classes("w-full gap-4"):
            ...
            with ui.row().classes("gap-2"):
                ui.button(
                    "Test Connectivity",
                    icon="network_check",
                    on_click=self.test_connectivity,
                )
    ...
    async def load_docker_status(self):
        ...
        try:
            ...
            if self.docker_list:
                self.docker_list.clear()
                with self.docker_list:
                    ui.label(f"Error loading Docker status: {str(e)}").classes(
                        "text-red-500"
                    )
